...
export default function AdminPage() {
  ...
  async function fetchOrders(targetPage = page, targetStatus = filterStatus) {
    try {
      setLoading(true);
      setError("");
      ...
      const res = await fetchWithTimeout(`/api/admin/orders?${params.toString()}`, {
        cache: "no-store",
      });
  ...
  async function updateStatus(orderId, status) {
    try {
      ...
      const res = await fetchWithTimeout(`/api/admin/orders/${orderId}/status`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  useEffect(() => {
    ...
    return () => clearInterval(interval);
  }, [accessGranted, page, filterStatus]);
      ...
      <div className="admin-container">
        <div className="admin-topbar">
          <div>
            ...
            <h1 className="admin-title">Gracee Ville Kitchen N Events</h1>
            <p className="admin-copy">
              Manage orders, monitor progress and update statuses.
            </p>
          </div>

          <button className="admin-secondary-btn" onClick={() => fetchOrders(page, filterStatus)}>
            Refresh Orders
          </button>
        </div>
        ...
        <div className="admin-topbar" style={{ gap: "12px", alignItems: "center" }}>
          <div>
            <label className="admin-copy" htmlFor="status-filter">
              Filter by status
            </label>
            <select
              id="status-filter"
        ...
        <div className="admin-topbar" style={{ marginTop: "20px" }}>
          <button
